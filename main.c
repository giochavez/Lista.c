...
void *retirarElPrimerValor(Lista *pLista, void *pInfo,  int (igualdad) (void *d1,void *d2)) {
    ...
    /*
    pNodo = pLista->inicio;
    while (pNodo->siguiente != pLista->fin) {
        if (igualdad(pNodo->siguiente->informacion,pInfo)==1) {
            pAnterior = pNodo->siguiente;
            pNodo->siguiente = pNodo->siguiente->siguiente;
            res = pAnterior->informacion;
            free(pNodo);
            return res;
        }
        pNodo = pNodo->siguiente;
